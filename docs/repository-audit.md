...
| Ship pricing and stock transfers | Value hulls and each outfit type at their own ages, truncate once per outfit group, and consume the corresponding records on ship transactions. Variants use their stock chassis model and price. Track signed outfit quantities separately from used-stock ages, preserving upstream's distinction between whole-ship and individual outfit sales. Refresh purchase quotes after transactions, reject payment/stock overflow before changing state, and remove a redundant hardpoint rebuild. | Eleven new simulation cases cover included-outfit resale, mixed and missing ages, group rounding, stock exhaustion, variant migration, legacy buyback saves, departure and payment/stock limits. Two engine cases verify used purchase quotes and the sale confirmation for recently installed equipment. The shipyard runtime smoke now reloads and buys the used replacement, checks the charged price and transfer of its outfit records, then departs with saved cargo. |

Latest local validation: **1008 simulation tests, 46 engine tests, zero failures or
...
save records remain readable. Signed quantities are saved under `stock`; ages of
sold hulls and outfits use `stock depreciation`. The former `outfit stock` records
migrate to both quantities and ages, and variant purchase keys migrate to their chassis model.
An ordinal `day` record can represent a fully depreciated item before year 1.
Older saves cannot recover purchase ages that were never recorded for included
outfits; those items retain the upstream unknown-age depreciation floor.
...
- **Economy:** individual port services and per-ship landing clearance remain
  ...
  automaton crew rules remain incomplete. Ship transactions still need chassis-only
  sales with outfit storage and trade-in.
