...
| M6 Fleet gameplay | **Partial; boarding and capture still need implementation** | Multiple owned ships with a shared player faction, fleet commands (escort/gather/hold/attack on upstream's `MoveTo` + `StoppingPoint`), salaries, cargo distribution, parking and flagship selection. Owned escorts still need runtime rendering and travel integration. Capture odds exist; boarding approach, crew combat, plunder, capture transfer and their UI remain incomplete. |
