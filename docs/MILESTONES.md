...
| M4 Landing economy | **Playable; the opening debt is not built** | Commodity/TradeData/CargoHold/Outfitting, local commodity transactions with saved average purchase costs and profit reporting, ships and outfits with upstream's `Depreciation`, daily sales, normal production shocks, linked-system exchanges and saved market state. Cargo pools ashore and survives ship sales and reloads; departure loads freight first and asks before selling excess goods or abandoning uncarryable jobs. The shipyard selects individual local hulls, confirms sales and quotes equipped purchase prices. Selling the last ship, saving at port, buying a replacement and departing are playable. The outfitter selects local ships, sells unlisted installed equipment, services modified hulls and preserves used buyback stock through reload until departure. Individual port services, per-ship landing clearance, outfit storage/licences, separate hull/outfit depreciation on ship transactions and jettison/plunder cost accounting remain incomplete. |
| M5 Missions | **Playable; additional gates and payloads remain incomplete** | Parsing, conditions, conversations (inline and top-level), events, NPC instances and saved objective history, accept/carry/complete/fail lifecycle with deadlines, and text substitution. Freight has a persistent identity per job and carrier, cannot be traded, must fit in full locally and must survive to the delivery system. NPC spawn/despawn gates, waypoints, stopovers, passenger distribution and full upstream save compatibility remain incomplete. |
| M6 Fleet gameplay | **Owned escorts playable; boarding and capture still need implementation** | Owned escorts render, fight, obey follow/gather/hold/attack orders and route after the flagship using their own drives and fuel. Jumps and each cargo carrier survive reloads. Shared player faction, salaries, cargo distribution, parking and flagship selection are integrated. Independent landing/refuelling, fighter operations, named formations and selected-group orders remain incomplete. Capture odds exist; boarding approach, crew combat, plunder, capture transfer and their UI remain incomplete. |
