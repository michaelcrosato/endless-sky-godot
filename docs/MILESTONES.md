...
| M4 Landing economy | **Playable; the opening debt is not built** | Commodity/TradeData/CargoHold/Outfitting, local commodity transactions with saved average purchase costs and profit reporting, ships and outfits with upstream's `Depreciation`, daily sales, normal production shocks, linked-system exchanges and saved market state. The landed screen has trade, shipyard, outfitter and job counters. Individual port services, per-ship landing clearance, landed cargo pooling and jettison/plunder cost accounting remain incomplete. |
