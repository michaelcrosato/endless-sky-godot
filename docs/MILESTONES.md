...
| M4 Landing economy | **Playable; the opening debt is not built** | Commodity/TradeData/CargoHold/Outfitting, `Trading` (commodity transactions using local fleet holds, plus ships and outfits with upstream's `Depreciation`), daily local supply drift (`StepEconomy`), and a landed screen with trade, shipyard, outfitter and job counters. Cost basis, linked markets and saved economy state remain incomplete. |
