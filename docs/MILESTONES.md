...
| M4 Landing economy | **Playable; the opening debt is not built** | Commodity/TradeData/CargoHold/Outfitting, `Trading` (commodity transactions using local fleet holds, plus ships and outfits with upstream's `Depreciation`), daily sales, normal production shocks and linked-system exchanges (`GameData.StepEconomy`), saved market state, and a landed screen with trade, shipyard, outfitter and job counters. Commodity cost basis, individual port services and per-ship landing clearance remain incomplete. |
